...
const ListedTag = ({tag, tagForColorPicker, handleTagClick, tagFilter, setTagFilter, setColorPickerMode}) => {
    ...
    const handleSaveEdit = () => {
        setEditMode(false)
        editTag({variables: {...tag, name: editTagName}})
    }
...
const NewTag = () => {
    ...
    const defaultTag = { name: 'New Tag', color: '#888888', favorite: false }
    ...
    if(create){
        return <div className="new-tag-create-mode">
            ...
            <div className="options">
                <img src={asset.redCross} alt="red cross" onClick={handleCancel}/>
                <img src={asset.thickGreenCheck} alt="green check" onClick={handleSave}/>
            </div>
        </div>
    }

    return <div className="new-tag" style={buttonStyles} onClick={()=>{
            setCreate(true)
        }}>
        <span>New Tag</span>
        <img src={asset.greenPlus} alt="green plus" style={{padding: '0 10px'}} />
    </div>
}
...
const TagFilter = ({colorPickerMode, setColorPickerMode, tagFilter, setTagFilter}) => {
    ...
    return (<>
        {colorPickerMode && <div className="color-picker-container module">
            <ColorPicker 
